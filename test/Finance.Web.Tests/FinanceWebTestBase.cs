...
{
    public abstract class FinanceWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static FinanceWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(FinanceWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<FinanceDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<FinanceDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<FinanceDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<FinanceDbContext, Task<T>> func)
        {
            T result;
